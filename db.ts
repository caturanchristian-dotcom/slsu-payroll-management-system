...
if (url || (host && user && database)) {
  ...
  useMysql = false;
  console.log("Database Mode: SQLite active (MySQL env vars missing or incomplete).");
}

// Lazy initialization of active connection
function getSqlite(): any {
  if (!sqliteDb) {
    sqliteDb = new SQLiteDatabase("payroll.db");
  }
  return sqliteDb;
}
...
// Execute queries robustly across both engines
...
async function executeQuery(sql: string, params: any[], type: "run" | "get" | "all") {
  if (useMysql) {
    ...
    // Convert boolean isSss/hasPhilhealth/hasPagibig integers if necessary
  ...
  transaction(fn: any) {
    return async (...args: any[]) => {
      if (useMysql) {
        ...
        const sdb = getSqlite();
        const runInTx = sdb.transaction(fn);
        return runInTx(...args);
      }
    };
  },
