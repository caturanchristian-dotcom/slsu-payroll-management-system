...
const DTR = () => {
  ...
  const handleCreateInlineSchedule = async () => {
    if (!inlineSchedule.subject || !inlineSchedule.startTime || !inlineSchedule.endTime) {
      const fieldLabel = selectedEmployee?.category === 'Regular Employee' ? 'Regular Type' : 'Subject';
  ...
  const fetchEmployees = async () => {
    try {
      ...
      if (data.length > 0 && !selectedEmployeeId) {
        ...
        const self = data.find((e: Employee) => e.email.toLowerCase() === user?.email.toLowerCase());
        setSelectedEmployeeId(self ? self.id : data[0].id);
      }
    } catch (error) {
  ...
  const getDayPunchesInner = (dayLogs: DTRLog[], dateStrOverride?: string) => {
    ...
    if (sorted.length >= 2) {
      ...
      // Temporary placeholder boundaries for schedule checking
      let resolvedAmStartTime = '08:00';
      let resolvedPmStartTime = '13:00';
      if (activeDateStr && selectedEmployee?.category === 'Regular Employee') {
        ...
        if (matchingSchedules.length > 0) {
          const sortedSchedules = [...matchingSchedules].sort((a, b) => a.startTime.localeCompare(b.startTime));
          if (sortedSchedules.length >= 2) {
            resolvedAmStartTime = sortedSchedules[0].startTime;
            resolvedPmStartTime = sortedSchedules[1].startTime;
            ...
            if (firstStartHour < 12) {
              resolvedAmStartTime = firstSched.startTime;
              resolvedPmStartTime = '00:00';
            } else {
              resolvedAmStartTime = '00:00';
              resolvedPmStartTime = firstSched.startTime;
            }
          }
        }
      }
      ...
      if (inDate) {
        ...
        if (activeDateStr && selectedEmployee?.category === 'Regular Employee') {
          if (resolvedAmStartTime !== '00:00' && resolvedPmStartTime === '00:00') {
            isAmShift = true;
          } else if (resolvedPmStartTime !== '00:00' && resolvedAmStartTime === '00:00') {
            isAmShift = false;
          } else {
            const getMinOfStr = (t: string) => {
              if (!t || t === '00:00') return 0;
              let [h, m] = t.split(':').map(Number);
              if (h > 0 && h <= 6) h += 12;
              return h * 60 + m;
            };
            const amMin = getMinOfStr(resolvedAmStartTime);
            const pmMin = getMinOfStr(resolvedPmStartTime);
            const logMin = hour * 60 + inDate.getMinutes();
            isAmShift = Math.abs(logMin - amMin) < Math.abs(logMin - pmMin);
          }
        } else {
          isAmShift = hour < 12;
        }
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                  ...
                  <h4 className="text-[11px] font-extrabold uppercase tracking-widest text-neutral-500">
                    {selectedEmployee?.category === 'Regular Employee' ? "Day's Work Schedules" : "Day's Class Schedules"}
                  </h4>
                </div>
              ...
              {getSchedulesForEditDate().length === 0 ? (
                <div className="text-center py-4 bg-white border border-dashed border-neutral-200 rounded-xl">
                  <p className="text-xs text-neutral-400 font-semibold italic">
                    {selectedEmployee?.category === 'Regular Employee' ? 'No work schedules for this day.' : 'No classes scheduled for this day.'}
                  </p>
                </div>
              ) : (
              ...
              <div className="pt-2 border-t border-dashed border-neutral-200">
                ...
                >
                  ...
                  {isAddingInlineSchedule 
                    ? (selectedEmployee?.category === 'Regular Employee' ? "Cancel New Schedule" : "Cancel New Class Schedule") 
                    : (selectedEmployee?.category === 'Regular Employee' ? "Add Specific Work Schedule for this Day" : "Add Specific Class Schedule for this Day")}
                </Button>
                ...
                {isAddingInlineSchedule && (
                  <div className="mt-3 p-3 bg-white border border-neutral-100 rounded-xl space-y-3">
                    <div className="text-[10px] font-extrabold uppercase tracking-wide text-neutral-400 text-left">
                      {selectedEmployee?.category === 'Regular Employee' ? 'New Schedule' : 'New Class'} for {editDate}
                    </div>

                    {selectedEmployee?.category === 'Regular Employee' && (
                    ...
                    <div className="space-y-1 text-left">
                      <Label htmlFor="inline-subject" className="text-[10px] font-semibold text-neutral-500">
                        {selectedEmployee?.category === 'Regular Employee' ? 'Regular Type' : 'Subject / Class Name'}
                      </Label>
                      <Input 
                        id="inline-subject" 
                        placeholder={selectedEmployee?.category === 'Regular Employee' ? 'e.g. Core Hours' : 'e.g. IT 101 - Intro'} 
                        className="h-8 rounded-lg text-xs" 
        ...
        <DialogContent className="sm:max-w-[425px] rounded-3xl no-print">
          ...
          <form onSubmit={handleManualSubmit} className="space-y-4 py-3 font-sans">
            ...
            {(() => {
              ...
              if (activeEmp?.category === 'Regular Employee') {
                return (
